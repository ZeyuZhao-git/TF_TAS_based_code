...
class EvolutionSearcher(object):
    ...
    def save_checkpoint(self):
        ...
        info['vis_dict'] = self.vis_dict #存了计算过的cands信息
    ...
    def calculate_rank_list(self, nums):
        sorted_nums = sorted(nums)  # 对数字列表进行排序
        t_list = []
        for num in nums:
            t_list.append(self.find_index(num,sorted_nums)+1)
        return t_list

    def find_index(self, number, sequence):
    ...
    def search(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            for indicator in self.indicator_names:
                indi_lists[indicator] = []
            for cand in self.candidates:
                self.memory[-1].append(cand)
                indicators = self.vis_dict[cand]['indicators']
                for key in indicators.keys():
                    indi_lists[key].append(indicators[key])
            
            for key in indi_lists.keys():
                rank_list = self.calculate_rank_list(indi_lists[key])
                rank_lists[key] = rank_list

            proxy_num = len(self.indicator_names)
            # proxy得分大的cand的'aggregate_rank'大
            i = 0
            for cand in self.candidates:
                tmp = 0
                for k in self.indicator_names:    
                    tmp = tmp + math.log(rank_lists[k][i]/proxy_num)
                i = i + 1
                self.vis_dict[cand]['aggregate_rank'] = tmp

            self.update_top_k(
                self.candidates, k=self.select_num, key=lambda x: self.vis_dict[x]['aggregate_rank']) #根据准确度更新排名前k的cands
            self.update_top_k(
                self.candidates, k=50, key=lambda x: self.vis_dict[x]['aggregate_rank'])

            print('epoch = {} : top {} result'.format(
                self.epoch, len(self.keep_top_k[50])))
            tmp_accuracy = []
            for i, cand in enumerate(self.keep_top_k[50]):
                print('No.{} {} Top-1 aggregate_rank = {}, params = {}'.format(
                    i + 1, cand, self.vis_dict[cand]['aggregate_rank'], self.vis_dict[cand]['params']))
                tmp_accuracy.append(self.vis_dict[cand]['aggregate_rank'])
...
def main(args):
    ...
    t = time.time()
    searcher = EvolutionSearcher(args, device, model, model_without_ddp, choices, data_loader_train, data_loader_val, data_loader_test, args.output_dir)
...
if __name__ == '__main__':
    ...
    args = parser.parse_args(["--data-path", "/data02/ImageNet2012",
                      ...
                      "--min-param-limits", "5",
                      "--param-limits", "7", 
                      "--output_dir","/home/zzy/TF_TAS/all_output/evo/SEED_3_50epoch",
                      "--resume-of-cands", "/home/zzy/TF_TAS/all_output/5/ranking_checkpoint-1000cands.pth",
                      '--indicator-name', 'zen', 'te_nas', 'NASWOT',
                      '--world_size','1','--seed','3','--max-epochs','50'
                      ])
